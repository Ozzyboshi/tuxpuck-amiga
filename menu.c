...
static void _menu_blit(Menu * menu, SDL_Rect * erase_rect) {
	...
	total_height += 2 * BORDER_SPACING + (menu->nbr_of_fields - 1)
			* FIELD_SPACING;
	rect.w = total_width;
	rect.h = total_height;
	...
	for (i = 0; i < menu->nbr_of_fields; i++) {
		...
		font_set_pen(_font,
		SCREEN_W / 2 - font_calc_width(_font, menu->text[i]) / 2, ruler_y);
...
int menu_get_selected(Menu * menu) {
	...
	printf("menu get selected 1\n");
	if (menu->selected == -1)
		_menu_select_next(menu);
	printf("menu get selected 2\n");
	_menu_blit(menu, &rect);
	printf("menu get selected 3\n");
	video_update();
	printf("menu get selected 4\n");
	while (loop) {
		while (SDL_PollEvent(&event)) {
			printf("menu get selected 5\n");
			switch (event.type) {
			case SDL_KEYDOWN:
	...
	printf("menu get selected 6\n");
	SDL_WM_GrabInput(SDL_GRAB_ON);
	printf("menu get selected 7\n");
	video_blit(background, &rect, &rect);
	printf("menu get selected 8\n");
