...
void run_intro(void) {
	...
	SDL_Rect text_rect, logo_rect,amiga_rect;
	Font *font= NULL;
	Font *font_amiga= NULL;
	Timer *timer= NULL;
	char txt_coder[] = "written by Jacob Kroon and Daniel Beck";
	char amiga_port[] = "Amiga porting by Alessio Garzi";
	char txt_gfx[] = "graphics by m0ns00n";
	char txt_version[] = "v" _VERSION;

#ifdef _DEBUG
	printf("Showing title.jpg file:'%s'\n",title_jpg);
#endif
	...
	font_amiga = font_create(monos_fnt, NULL);
	font_set_color(font_amiga, 170, 140, 110);
	font_set_pen(font_amiga, 70, 0 + sdl_logo->h);
...
#endif
			font_print(font_amiga, amiga_port, &amiga_rect);			
